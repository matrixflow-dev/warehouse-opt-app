...
void AgentLoader::loadKiva(const std::string fname, std::vector<unsigned int> capacity, const MapLoaderCost &ml)
{
    string line;
    ifstream myfile(fname.c_str());
    if (myfile.is_open())
    {
        ...
        for (int i = 1; i < rows - 1; i++)
        {
            ...
            for (int j = 1; j < cols - 1; j++)
            {
                ...
                {
                    agents.push_back(new Agent(num_of_agents, i * ml.cols + j, capacity[num_of_agents]));
                    num_of_agents++;
                }
            }
        }
