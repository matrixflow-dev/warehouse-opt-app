...
Task::Task(int id, int init_t, int init_loc, int goal_loc, int vol)
{
    task_id = id;
    initial_time = init_t;
...
TaskLoader::TaskLoader(const std::string fname, MapLoaderCost &ml)
{
    ...
    if (myfile.is_open())
    {
        ...
        for (int i = 0; i < num_of_tasks; i++)
        {
            ...
            c_beg++;
            int vol = atoi((*c_beg).c_str());
            Task *new_task = new Task(i, init_time, init_loc, goal_loc, vol);
...
void TaskLoader::loadKiva(const std::string fname, MapLoaderCost &ml)
{
    ...
    for (int i = 0; i < task_num; i++)
    {
        ...
        ss.clear();
        ss << line;
        ss >> t >> s >> g >> ts >> tg >> vol; // time+start+goal+time at start+time at goal
        s = s % ml.endpoints.size();
        g = g % ml.endpoints.size();
        assert(s < ml.endpoints.size());
        assert(g < ml.endpoints.size());
        Task *new_task = new Task(i, t, ml.endpoints[s], ml.endpoints[g], vol);
