...
int main(int argc, char **argv)
{
    ...
    desc.add_options()("help", "produce help message")("map,m", po::value<std::string>()->required(), "input file for map")("agents,a", po::value<std::string>()->required(), "initial locations for agents")("tasks,t", po::value<std::string>()->required(), "a set of tasks")("objective", po::value<std::string>()->default_value("total-travel-time"), "the objective of the task assignment")("screen", po::value<int>()->default_value(0), "screen option (0: none; 1: results; 2:all)")("output,o", po::value<std::string>()->required(), "output file for schedule")("solver,s", po::value<std::string>()->required(), "solvers (CBS, ICBS, CBSH, CBSH-CR, CBSH-R, CBSH-RM, CBSH-GR")("agentNum,k", po::value<int>()->default_value(0), "number of agents")("cutoffTime,c", po::value<float>()->default_value(7200), "cutoff time (seconds)")("capacity", po::value<vector<unsigned>>()->multitoken()->required(), "cutoff time (seconds)")("seed,d", po::value<int>()->default_value(0), "random seed")("corridor2", po::value<bool>(), "reason about 2-way branching corridor conflicts")("target", po::value<bool>(), "reason about target conflict")("kDelay", po::value<int>()->default_value(0), "generate k-robust plan")("window,w", po::value<int>()->default_value(0), "The window size of windowed cbs")("RM-4way", po::value<int>()->default_value(1), "0, do not do 4 way split. 1, do 4 way only necessary.2 Always do 4 way splitting for RM, other wise only 4 way when necessary")("f_w", po::value<float>()->default_value(1), "suboptimal bound")("statistic", "print statistic data")("only-assignment", "only do task assignment")("multi-label", "search for multiple goals")("no-task-loop", "do not loop over all unassigned tasks")("real-cost", "use conflict free cost")("only-update-top", "only update top assign")("real-cost-insert", "use conflict free cost")("task", "marginal cost on task")("regret", "regret based")("regret-task", "regret on task not agent")("anytime", "continue optimze until time-limit")("group-size", po::value<int>()->default_value(1), "anytime group_size")("max-iteration", po::value<int>()->default_value(10000), "max_iteration")("anytime-log", po::value<std::string>()->default_value(""), "output file for anytime log")("destory-method", po::value<std::string>()->default_value("destory-max"), "destory-max or random")("online", "online mode")("kiva", "load kiva map and tasks");
    ...
    if (vm.count("output"))
    {
        ...
        output = "";
    auto capacity = vm["capacity"].as<vector<unsigned>>();
    ...
    else
    {
        ...
        if (screen >= 1)
        {
            cout << "task_assignment:\n";
            taskAssignment->printAssignments();
            cout << "path_for_each_agent:\n";
            taskAssignment->printPath();
        }
        ...
        if (options1.s == constraint_strategy::PP || vm.count("only-assignment"))
        {
            ...
            cout << "Number agents: " << agentLoader->num_of_agents << " Number tasks: " << taskLoader->num_of_tasks
                 ...
                 << ", Real dealy: " << total_delay << ", TA Runtime: " << taskAssignment->runtime / CLOCKS_PER_SEC << ", No of agents with tasks: " << taskAssignment->get_num_agents_with_tasks() << endl;
            cout << "runtime_pp: " << taskAssignment->runtime_pp / CLOCKS_PER_SEC << " runtime_update_conflict: " << taskAssignment->runtime_update_conflict / CLOCKS_PER_SEC << " runtime_update_changed_agent: " << taskAssignment->runtime_update_changed_agent / CLOCKS_PER_SEC << " num_conflict_updates: " << taskAssignment->num_conflict_updates << " num_task_assign_updates: " << taskAssignment->num_task_assign_updates << endl;
        ...
        else
        {
            ...
            cout << "Ideal total delay: " << ideal_delay << ", Final delay: " << final_delay << ", Ideal cost: " << ideal_cost << ", Final_cost: " << final_cost << ", Cost increase: "
                 << ", ideal Makespan: " << taskAssignment->current_makespan << ", Makespan" << makespan << ", TA Runtime: " << taskAssignment->runtime / CLOCKS_PER_SEC << ", Path Planning Runtime: " << planner->runtime << ", No of agents with tasks: " << taskAssignment->get_num_agents_with_tasks() << endl;

            if (output != "")
            {
                ofstream stats;
                stats.open(output, ios::app);
                stats << success << "," << agentLoader->num_of_agents << "," << taskLoader->num_of_tasks << "," << ideal_cost << "," << final_cost << "," << ideal_delay << "," << final_delay << ","
